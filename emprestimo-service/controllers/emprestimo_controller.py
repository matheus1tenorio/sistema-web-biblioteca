...
CLIENTE_SERVICE_URL = os.getenv("CLIENTE_SERVICE_URL", "http://cliente-service:5000")
...
def criar_emprestimo(data):
    ...
    try:
        resp_cliente = requests.get(f"{CLIENTE_SERVICE_URL}/clientes/{cliente_id}", timeout=5)
        if resp_cliente.status_code == 404:
            return {"erro": "Cliente não encontrado"}, 404

        resp_livro = requests.get(f"{LIVRO_SERVICE_URL}/livros/{livro_id}", timeout=5)
        ...
        return {"erro": f"Erro ao validar cliente ou livro: {str(e)}"}, 500
